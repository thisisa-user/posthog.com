...
The Postgres connector links your database tables to PostHog. You can sync data into PostHog's managed storage or query your Postgres database directly from the SQL editor. For synced data, you can use regular scheduled syncs or enable change data capture (CDC) for near real-time syncs that capture inserts, updates, and deletes.
...
6. If you selected **Sync to warehouse**, optionally enable **Change data capture (CDC)** if your database is prepared for logical replication.
7. Click **Next**, select the tables to sync, choose the sync method for each table, then click **Link**
